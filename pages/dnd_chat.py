...
st.title("🐉 D&D Rules")
st.markdown("Je connais les règles basiques de la 5e édition, n'hésite pas si une notion te semble complexe ou si tu as besoin d'aide pour créer un personnage.")

# ============= INIT RAG =============
@st.cache_resource
def load_rag_vectorstore():
    """Charger le PDF D&D automatiquement"""
    pdf_path = "C:\\Users\\Utilisateur\\Desktop\\projet_dnd\\D-D_AI_Companion\\rag\\basic-rules-fr.pdf"  # À adapter avec votre chemin
    
    if not os.path.exists(pdf_path):
        st.warning(f"⚠️ PDF non trouvé: {pdf_path}")
        return None
    
    try:
        with st.spinner("📚 Chargement du document D&D..."):
            with open(pdf_path, 'rb') as f:
                vector_store, embeddings = process_pdf_to_vectorstore(f)
            st.sidebar.success("✅ Document D&D chargé!")
            return vector_store
    except Exception as e:
...
def generate_response(input_text):
    ...
    try:
        ...
        rag_context = ""
        if rag_vector_store:
            context = retrieve_from_rag(input_text, rag_vector_store, k=3)
            if context:
                rag_context = context
...
- Sinon, utilise un tool du serveur MCP.
